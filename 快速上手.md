...
这份快速上手说明如何安装 Codex Harness Runner，并为一个项目配置 harness 多智能体工作流。
...
需要让助手配置本地 harness workflow 时，可以使用下面这段 prompt：
...
请完成：
1. 阅读 runner 的 README、快速上手.md 和 profiles/example.toml。
...
不要在同一个 `profile.cwd` 上并行运行多个 harness/Codex CLI 写任务。需要并行时，为每个任务创建独立 Git worktree，并配置独立 profile。
